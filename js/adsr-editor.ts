...
class ADSREditor {
  ...
  dragging: ADSRParam | null;
  attack: number;
  decay: number;
  sustain: number;
  release: number;
  ranges: Record<ADSRParam, { min: number; max: number }>;
  pad: { top: number; bottom: number; left: number; right: number };
  // ドラッグ中に固定するスケール情報
  _dragScale: number | null = null;
  _dragX0 = 0;
  _dragY0 = 0;
  _dragH = 0;
  ...
  init() {
    this.canvas = document.getElementById('adsr-canvas') as HTMLCanvasElement | null;
  ...
  valToNorm(param: ADSRParam, val: number) {
    const r = this.ranges[param];
    return (val - r.min) / (r.max - r.min);
  }

  normToVal(param: ADSRParam, norm: number) {
    const r = this.ranges[param];
  ...
  hitTest(pos: { x: number; y: number }): ADSRParam | null {
    const p = this.getPoints();
    const threshold = 12;
...
export const adsrEditor = new ADSREditor();
