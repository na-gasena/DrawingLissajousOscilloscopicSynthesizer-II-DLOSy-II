...
class UIComponents {
  ...
  onKnobMouseMove(e: MouseEvent) {
    ...
    if (this.activeKnob === 'tempo') {
      ...
      document.getElementById("tempo-value")!.textContent = String(Math.round(newValue));
      if (stepSequencer) stepSequencer.updateTempo();
    } else if (this.activeKnob === 'masterFreqShift') {
      audioEngine.params.masterFreqShift = newValue;
      if (stepSequencer) stepSequencer.masterFreqShift = newValue;
    } else {
  ...
  // ===== WAVE BUTTONS =====
  initWaveButtons() {
    document.querySelectorAll<HTMLElement>('.wave-btn').forEach(btn => {
  ...
  async onKeyDown(note: string, keyEl: Element) {
    ...
    if (audioEngine.params.waveType === 'drawing' && drawingMode) {
  ...
  initKeyboardInput() {
    ...
    const allMap: Record<string, string> = { ...whiteMap, ...blackMap };
  ...
  cycleWaveType() {
    ...
    document.querySelectorAll<HTMLElement>('.wave-btn').forEach(b => {
