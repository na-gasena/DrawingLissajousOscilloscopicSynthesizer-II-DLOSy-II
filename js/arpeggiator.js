...
class Arpeggiator {
  ...
  setDiv(div) {
    ...
    document.querySelectorAll('.arp-div-btn').forEach(b => {
      b.classList.toggle('active', parseInt(b.dataset.div, 10) === div);
    });
    const input = document.getElementById('arp-div-input');
    if (input && parseInt(input.value, 10) !== div) input.value = div;
  }

  // ===== CHORDS & PROGRESSIONS =====

  // Replace the held set with a chord and HOLD it (latch on)
  applyChordNotes(notes) {
  ...
  stopProgression() {
    ...
    this.progId = null;
    const sel = document.getElementById('arp-prog-select');
    if (sel) sel.value = '';
  }
  ...
  _scheduleProgAdvance() {
    this._clearProgTimer();
    const bpm = (window.audioEngine && audioEngine.params.tempo) ? audioEngine.params.tempo : 120;
  ...
  bindUIEvents() {
    ...
    document.querySelectorAll('.arp-div-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        this.setDiv(parseInt(btn.dataset.div, 10));
      });
    });
    ...
    document.getElementById('arp-chord-root')?.addEventListener('change', (e) => {
      this.setChordRoot(parseInt(e.target.value, 10));
    });
    document.querySelectorAll('.arp-chord-btn').forEach(btn => {
    ...
    document.getElementById('arp-prog-bars')?.addEventListener('change', (e) => {
      this.barsPerChord = parseInt(e.target.value, 10) || 1;
